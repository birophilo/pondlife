...
def get_spritesheet(id: str, request: Request):
    mongo_client = MongoCRUDClient()
...
async def update_spritesheet(id: str, request: Request):
    ...
    if len(spritesheet) >= 1:
        mongo_client = MongoCRUDClient()
        update_result = mongo_client.update_document("spritesheets", spritesheet)
...
def delete_spritesheet(id: str, request: Request, response: Response):
    mongo_client = MongoCRUDClient()
